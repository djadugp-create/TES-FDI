...
void MatrixPanel_I2S_DMA::clearFrameBuffer(bool _buff_id){
  ...
  do {
    ...
    if ( m_cfg.driver == HUB75_I2S_CFG::SM5266P) {  
        ...
        do{
            serialCount--;
            latch = row[x_pixel] | (((((ESP32_I2S_DMA_STORAGE_TYPE)row_idx) % 16) == serialCount) << 1) << BITS_ADDR_OFFSET; // data on 'B'
            row[x_pixel++] = latch| (0x0C<< BITS_ADDR_OFFSET); // clock high on 'A'and BK high for update
            row[x_pixel++] = latch| (0x30<< BITS_ADDR_OFFSET); // clock low on 'A'and BK high for update
        } while (serialCount);
    } // end SM5266P    
...
//  l = (((x_coord % l) + l) >=PIXELS_PER_ROW ) ? (2*PIXELS_PER_ROW - x_coord):l;
